...
Future<Uint8List> exportQuickNotesToPdf(
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (pw.Context context) {
        List<pw.Widget> elements = _buildAppendixHeader(
          'APPENDIX A: GLOSSARY OF TERMS',
          'appendix_a',
          boldUnicodeFont,
        );
        ...
        for (var entry in termsBySubunit.entries) {
          elements.add(
            _buildAppendixSubhead(
              '${entry.key.id} ${entry.key.title}',
              boldUnicodeFont,
            ),
          );
          ...
          for (int i = 0; i < entry.value.length; i++) {
            ...
            final bool isHL = term.tags != null
            ...
            termRows.add(
              pw.TableRow(
                ...
                children: [
                  _buildTableCell(
                    isHL ? '[HL] ${term.termName}' : term.termName,
                    boldUnicodeFont,
                    isHL ? PdfColors.purple900 : PdfColors.indigo900,
                  ),
                  _buildTableCell(
          ...
          elements.add(
            pw.Wrap(
              ...
              children: entry.value.entries.map((diagEntry) {
                ...
                const double renderSize =
                    145.0; // Adjusted optimally for 48px margins
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      pw.Container(
                        ...
                        child: pw.Text(
                          isHL
                              ? '[HL] ${_formatEnumName(diagEnum.name)}'
                              : _formatEnumName(diagEnum.name),
                          textAlign: pw.TextAlign.center,
                          style: pw.TextStyle(
                            font: boldUnicodeFont,
                            fontSize: 9,
  ...
  for (var block in slide.contents!) {
    ...
    if (block.content != null && block.content!.text.isNotEmpty) {
      final bool blockIsHL = block.content!.tags != null
          ? HlStyle.hasHL(block.content!.tags!)
          : false;
      final List<pw.Widget> htmlWidgets = HtmlPdfBuilder.build(
        block.content!.text,
    ...
    if (block.tableData != null) {
      ...
      if (table.title != null && table.title!.isNotEmpty) {
        final bool isHL = table.tags != null
      ...
      for (int i = 0; i < table.headers.length; i++) {
        columnWidths[i] = pw.FlexColumnWidth(
          (table.flexColumnWidths != null && i < table.flexColumnWidths!.length)
...
pw.Widget _buildTocAppendixLink(
  String title,
  String destination,
  pw.Font font,
) {
...
List<pw.Widget> _buildAppendixHeader(
  String title,
  String anchorName,
  pw.Font font,
) {
...
pw.Widget _buildAppendixSubhead(String title, pw.Font font) {
  return pw.Padding(
    ...
    child: pw.Text(
      title,
      style: pw.TextStyle(font: font, fontSize: 14, color: PdfColors.indigo700),
    ),
  );
}
...
pw.Widget _buildFooter(pw.Context context, pw.Font font, String sectionTitle) {
  return pw.Container(
    ...
    decoration: const pw.BoxDecoration(
      border: pw.Border(
        top: pw.BorderSide(color: PdfColors.grey400, width: 1.2),
      ),
    ),
    child: pw.Column(
      children: [
...
pw.Widget buildPdfDiagramRow(
  ...
  double renderSize = totalDiagrams <= 2
      ? 220.0
      : (totalDiagrams == 3 ? 145.0 : 120.0);
