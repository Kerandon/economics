...
Future<void> handlePdfExport(BuildContext context, PaperQuestion question) async {
  ScaffoldMessenger.of(context).showSnackBar(
    const SnackBar(content: Text('Generating Full PDF...'), duration: Duration(seconds: 1)),
  );

  // Hydrate all diagrams for the whole page ahead of time using your existing class
  final size = MediaQuery.of(context).size;
  final theme = Theme.of(context);
  final allDiagramsService = AllDiagrams(size: size, colorScheme: theme.colorScheme);

  await exportFullQuestionToPdf(question, allDiagramsService);
}
